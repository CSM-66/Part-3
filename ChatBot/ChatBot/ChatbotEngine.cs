...
namespace ChatBOT.ChatBot
{
    ...
    public class ChatbotEngine
    {
        ...
        // NLP phrase variations maps different ways of saying something to a keyword
        private readonly Dictionary<string, string> _nlpPhrases = new()
        {
            ["how do i create a strong password"] = "password",
            ["how to make a password"] = "password",
            ["what makes a good password"] = "password",
            ["i forgot my password"] = "password",
            ["password tips"] = "password",
            ["how do i spot a phishing email"] = "phishing",
            ["how to avoid phishing"] = "phishing",
            ["what is a phishing attack"] = "phishing",
            ["suspicious email"] = "phishing",
            ["fake email"] = "phishing",
            ["how to stay safe online"] = "safe browsing",
            ["how do i browse safely"] = "safe browsing",
            ["safe internet tips"] = "safe browsing",
            ["internet safety"] = "safe browsing",
            ["how to protect my privacy"] = "privacy",
            ["how do i keep my data private"] = "privacy",
            ["what is privacy"] = "privacy",
            ["protect my data"] = "privacy",
            ["what is malware"] = "malware",
            ["how do i remove malware"] = "malware",
            ["my computer has a virus"] = "malware",
            ["virus on my computer"] = "malware",
            ["what is a vpn"] = "vpn",
            ["how does a vpn work"] = "vpn",
            ["do i need a vpn"] = "vpn",
            ["what is two factor"] = "two factor",
            ["how does 2fa work"] = "two factor",
            ["enable two factor"] = "two factor",
        ...
        public (string response, ResponseType type) ProcessInput(string input)
        {
            ...
            // Activity log
            if (cleaned.Contains("show full log") || cleaned.Contains("full activity log"))
            {
                return (GetActivityLog(true), ResponseType.Normal);
            }

            if (cleaned.Contains("show activity log") || cleaned.Contains("what have you done") || cleaned.Contains("activity log") || cleaned.Contains("what did you do"))
            {
                return (GetActivityLog(), ResponseType.Normal);
            }
            ...
            // NLP phrase matching — check for natural language variations first
            foreach (var phrase in _nlpPhrases.Keys)
            {
                if (cleaned.Contains(phrase))
                {
                    string mappedKeyword = _nlpPhrases[phrase];
                    _memory.LastTopic = mappedKeyword;
                    string nlpTip = GetRandomResponse(mappedKeyword);
                    string nlpResponse = BuildPersonalisedResponse(mappedKeyword, sentimentPrefix + nlpTip);
                    ResponseType nlpType = string.IsNullOrEmpty(sentimentPrefix) ? ResponseType.Normal : ResponseType.Sentiment;
                    return (nlpResponse, nlpType);
                }
            }
        ...
        public string GetActivityLog(bool showAll = false)
        {
            if (_activityLog.Count == 0)
                return "No activity recorded yet.";

            int limit = showAll ? _activityLog.Count : Math.Min(5, _activityLog.Count);
            string log = $"Here's a summary of recent actions ({_activityLog.Count} total):\n";

            for (int i = 0; i < limit; i++)
                log += $"  {i + 1}. {_activityLog[i]}\n";

            if (!showAll && _activityLog.Count > 5)
                log += $"\n  ...and {_activityLog.Count - 5} more. Type 'show full log' to see all.";
