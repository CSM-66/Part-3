...
";
            ...
            pnlTopics.Size = new Size(860, 80);
            pnlTopics.FlowDirection = FlowDirection.LeftToRight;
            pnlTopics.WrapContents = true;
            pnlTopics.AutoScroll = false;

            string[] topics = { "Password", "Phishing", "Scam", "Privacy", "Malware", "Safe Browsing", "VPN", "Two Factor", "Tasks", "Quiz" };
            Color[] btnColors = {
        ...
        Color.FromArgb(140, 100, 0)
            ...
            for (int i = 0; i < topics.Length; i++)
            {
                ...
                btn.Click += (s, e) =>
                {
                    ...
                    {
                        txtInput.Text = $"Tell me about {topic.ToLower()}";
                        SendMessage();
                    }
                };
        ...
        private void SendMessage()
        {
            ...
            AppendUserMessage(input);
            // Open Task Form
            if (input.ToLower().Contains("task") || input.ToLower().Contains("add task") || input.ToLower().Contains("view tasks") || input.ToLower().Contains("my tasks"))
            {
                _engine.LogAction("User opened the Task Assistant.");
                new TaskForm().ShowDialog();
                return;
            }

            // Open Quiz Form
            if (input.ToLower().Contains("start quiz") || input.ToLower().Contains("quiz"))
            {
                _engine.LogAction("User started the Cybersecurity Quiz.");
            ...
            AppendLine("");
            _engine.LogAction($"User asked about: {input}");
            UpdateMemoryBar();
        }
